...
def count_bad_actors
  MovieDatabase.execute(<<-SQL)
  SELECT
    count(actor.id) as num_bad_actors
  FROM
    actor
  LEFT OUTER JOIN
...
def chris_is_missed
  ...
  JOIN
    actor ON casting.actorid = actor.id
  JOIN
    casting cfc ON cfc.movieid = movie.id
  JOIN
    actor cf ON cfc.actorid = cf.id
  WHERE
    casting.ord = 1
  AND
    cf.name = 'Chris Farley'
  SQL
end
