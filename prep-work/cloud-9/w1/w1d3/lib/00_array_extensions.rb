...
class Array
  def my_uniq
    unique = []
    self.each{|x| unique.push(x) if !unique.include?(x)}
    unique.sort!
  end
end
...
class Array
  def two_sum
    ...
    self.each_with_index do |number, index|
      (index+1..self.length-1).each do |next_index|
        pairs.push([index, next_index]) if (number + self[next_index]) == 0
      end
    end
    pairs
  end
...
class Array
  def median
    ...
    n = arr.length
    return nil if n < 1
    return arr[n/2] if n % 2.0 == 1
    0.5*(arr[n/2] + arr[n/2 - 1])
  end
end
