...
class MyHashSet
  ...
  def include?(el)
    store.include?(el) and store[el] == true
  end
  ...
  def to_a
    arr = []
    store.each{|key, value| arr.push(key)}
    arr
  end
  
  def union(set2)
    new_my_hash_set = MyHashSet.new
    new_my_hash_set.store = self.store.merge(set2.store)
    new_my_hash_set
  end
  
  def intersect(set2)
    new_my_hash_set = MyHashSet.new
    store.each{ |el, value| new_my_hash_set.insert(el) if set2.include?(el)}
    new_my_hash_set
  end
